...
export const db = {
    ...
    searchUsers: async (query) => {
        // Allow letters, numbers, spaces, @, ., - and _ for email/username search
        ...
        const { data, error } = await supabase
            ...
            .or(`username.ilike.%${safeQuery}%,display_name.ilike.%${safeQuery}%,email.ilike.%${safeQuery}%`)
            .limit(20);
        return { data, error };
    },
    ...
    acceptFriendRequest: async (requestId) => {
        ...
        // Use upsert with ignoreDuplicates to avoid unique constraint errors if row already exists
        ...
        if (reverseError) {
            console.error('Error creating reverse friendship (non-fatal):', reverseError);
