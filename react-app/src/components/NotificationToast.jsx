...
export default function NotificationToast({ notification, onClose, onAcceptFriendRequest }) {
  ...
  const totalTime = notification.duration || (['info', 'success', 'chat_message', 'friend_accepted'].includes(notification.type) ? 5000 : 10000);
  ...
  const handleAccept = async () => {
    if (notification.type === 'party_invite') {
      const contentType = notification.content_type || 'movie';
      const season = notification.season;
      const episode = notification.episode;
      let url = `/watch-party-room?room=${notification.room_id}&movie=${notification.movie_id || 'big-buck-bunny'}&type=${contentType}&host=false`;
