...
export function ProfileProvider({ children }) {
  ...
  // Google OAuth users bypass auth.signUp and never get a profiles row created,
  // making them invisible to friend search. This creates one on first login.
  const ensureProfile = useCallback(async () => {
    ...
    try {
      ...
      // Derive best-guess name from Auth metadata
      const meta = user.user_metadata || {};
      const oauthName = meta.full_name || meta.name || meta.display_name || '';
      const oauthUsername = meta.preferred_username || meta.user_name || user.email?.split('@')[0] || '';
      const oauthAvatar = meta.avatar_url || meta.picture || '';

      if (!existing) {
        // Profile row doesn't exist — create it
        console.log('📝 Creating missing profiles row for user:', user.id);
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            display_name: oauthName || oauthUsername,
            avatar_url: oauthAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.email}`,
            online_status: false,
        ...
        if (insertError) {
          // Duplicate key means another tab/request already created it — that's fine
          if (insertError.code !== '23505') {
            console.error('Failed to create profiles row:', insertError);
          }
        } else {
          console.log('✅ Profiles row created for:', oauthName || oauthUsername);
        }
      } else {
        // Profile row exists but display_name might be empty (e.g. old accounts)
        // Sync from OAuth metadata if profile has no display_name
        const needsSync = (!existing.display_name && oauthName) ||
          (!existing.username && oauthUsername);

        if (needsSync) {
          console.log('🔄 Syncing display_name from Auth metadata for user:', user.id);
          const updates = {};
          if (!existing.display_name && oauthName) updates.display_name = oauthName;
          if (!existing.username && oauthUsername) updates.username = oauthUsername;
  ...
  const loadProfileData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // FIRST ensure the profiles row exists (for Google OAuth users)
      await ensureProfile();
