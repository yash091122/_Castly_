...
import { usePartyDataChannel } from '../hooks/usePartyDataChannel';
...
const VideoTile = ({ peer, stream, participantName = "Participant", participantId, avatar, isAudioEnabled = true, isVideoEnabled = true }) => {
  ...
  useEffect(() => {
    ...
    if (stream && ref.current && isMountedRef.current) {
      if (ref.current.srcObject !== stream) {
        ref.current.srcObject = stream;
        // Only play if element is still in document
        if (document.contains(ref.current)) {
          ref.current.play().catch(e => {
            // Ignore errors if element was removed
            if (!e.message.includes('removed from the document')) {
              console.warn('VideoTile prop stream play failed:', e);
            }
          });
        }
      }
  ...
  useEffect(() => {
    ...
    const handleMetadata = (e) => {
      ...
      if (e.target && e.target.play && document.contains(e.target)) {
        e.target.play().catch(err => {
          if (!err.message.includes('removed from the document')) {
            console.warn('Play blocked on loadedmetadata:', err);
          }
        });
      }
    };
    ...
    const handleCanPlay = (e) => {
      ...
      if (e.target && e.target.play && document.contains(e.target)) {
        e.target.play().catch(err => {
          if (!err.message.includes('removed from the document')) {
            console.warn('Play blocked on canplay:', err);
          }
        });
      }
    };
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        if (videoEl.paused) {
          console.log('👁️ VideoTile visible, attempting to play', participantName);
          videoEl.play().catch(e => console.warn('VideoTile visibility play failed:', e));
        }
      }
    };
...
function WatchPartyRoom() {
  ...
  const { socket, getSocket, onlineUsers, reconnectCount } = useSocketContext();
  ...
  // ── DataChannel layer (browser gives WebRTC media priority, survives tab switches) ──
  const {
    attachToPeer,
    dcSend,
    dcOn,
    dcOff,
    removePeer: dcRemovePeer,
    openCount: dcOpenCount
  } = usePartyDataChannel(peersRef);

  // ── Unified room emit: DataChannel first, Socket.io fallback ──────────
  // DataChannels are not throttled in background tabs; Socket.io is.
  // Once peers are connected, all real-time room events go P2P via DC.
  const roomEmit = useCallback((eventType, payload) => {
    if (dcOpenCount() > 0) {
      dcSend(eventType, payload);
    } else {
      // No peers connected yet (or solo room) — fall back to server relay
      socket?.emit(eventType, { roomId, ...payload });
    }
  }, [socket, roomId, dcSend, dcOpenCount]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const isSyncingRef = useRef(false); // Prevent play/pause conflicts during sync
  ...
  useEffect(() => {
    ...
    const getLatestPeerId = (originalId, peerInstance) => {
      const p = peersRef.current.find(p => p.peer === peerInstance);
      return p ? p.socketId : originalId;
    };
    ...
    const addPeer = (socketId, peer, name, odId = null, avatar = null, isInitiator = false) => {
      ...
      // ── Attach DataChannel so this peer bypasses Socket.io for in-room events ──
      attachToPeer(socketId, peer, isInitiator);
    ...
    const createPeer = (targetSocketId, initiator, peerName) => {
      ...
      const streamToSend = streamRef.current || null;
      if (!streamToSend) {
      ...
      peer.on("signal", signal => {
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log('📡 Sending signal to:', currentTargetId, 'type:', signal.type, 'myName:', myName);
        ...
        socket.emit("signal", {
          to: getLatestPeerId(targetSocketId, peer),
          from: socket.id,
          signal,
          userName: myName,
          userId: user?.id,
      ...
      peer.on('stream', remoteStream => {
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log('📺 RECEIVED STREAM from:', currentTargetId, 'streamId:', remoteStream.id, 'tracks:', remoteStream.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled })));
        // Update both ref and state
        const peerRef = peersRef.current.find(p => p.socketId === getLatestPeerId(targetSocketId, peer));
        if (peerRef) {
        ...
        setPeers(prev => {
          const updated = prev.map(p =>
            p.socketId === getLatestPeerId(targetSocketId, peer) ? { ...p, stream: remoteStream } : p
          );
      ...
      peer.on('track', (track, remoteStream) => {
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log('🎵 RECEIVED TRACK from:', currentTargetId, track.kind, 'enabled:', track.enabled, 'readyState:', track.readyState);
        if (remoteStream) {
          // Update both ref and state
          const peerRef = peersRef.current.find(p => p.socketId === getLatestPeerId(targetSocketId, peer));
          if (peerRef) {
      ...
      peer.on('connect', () => {
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log('✅ PEER CONNECTED:', currentTargetId);
      ...
      peer.on('error', err => {
        ...
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.error('❌ Peer Error:', currentTargetId, err.message, err);
        ...
        if (err.message.includes('Connection failed') || err.message.includes('setRemoteDescription')) {
          console.log('🧹 Cleaning up failed peer:', currentTargetId);
          try {
            peer.destroy();
          } catch (e) {
            console.log('  Peer already destroyed');
          }
          peersRef.current = peersRef.current.filter(p => p.socketId !== getLatestPeerId(targetSocketId || p.socketId, p.peer || peer));
      ...
      peer.on('close', () => {
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log('🔌 Peer Closed:', currentTargetId);
      ...
      peer._pc.addEventListener('iceconnectionstatechange', () => {
        ...
        const currentTargetId = getLatestPeerId(targetSocketId, peer);
        console.log(`🧊 ICE Connection State (${currentTargetId}):`, state);

        if (state === 'failed') {
          console.warn(`⚠️ ICE connection failed for ${targetSocketId}, scheduling recovery`);

          let oldOdId = null;
          let oldAvatar = null;
          const oldPeerData = peersRef.current.find(p => p.socketId === targetSocketId);
          if (oldPeerData) {
            oldOdId = oldPeerData.odId;
            oldAvatar = oldPeerData.avatar;
          }
          ...
          if (shouldInitiatePeer(getLatestPeerId(targetSocketId, peer))) {
            setTimeout(() => {
              if (hasJoinedRoom.current && socket.connected) {
                console.log(`🔄 Attempting to recreate peer for ${targetSocketId} after ICE failure`);
                const resolvedId = getLatestPeerId(targetSocketId, peer);
                const newPeer = createPeer(resolvedId, true, finalName);
                if (newPeer) addPeer(resolvedId, newPeer, finalName, oldOdId, oldAvatar, true);
              }
            }, 2000);
          }
        }
    ...
    const handleExistingParticipants = (existingParticipants) => {
      ...
      existingParticipants.forEach(({ socketId, odId, userData }) => {
        ...
        if (shouldInitiatePeer(socketId)) {
          // Only initiate if we don't already have one (e.g. initial load or actual recovery after failure)
          if (!peerExists(socketId)) {
            console.log(`🔄 Attempting to initiate WebRTC with ${peerName} (${socketId})`);
            const peer = createPeer(socketId, true, peerName);
            if (peer) {
              addPeer(socketId, peer, peerName, odId, peerAvatar, true);
            }
          }
        }
      });
    ...
    const handleParticipantJoined = ({ userData, socketId }) => {
      ...
      if (socketId && socketId !== socket.id && !peerExists(socketId) && shouldInitiatePeer(socketId)) {
        const peer = createPeer(socketId, true, peerName);
        if (peer) {
          addPeer(socketId, peer, peerName, userData?.id || null, userData?.avatar || null, true);
        }
      }
    };

    const handleParticipantRecovered = ({ socketId, userData }) => {
      console.log('🔄 Participant recovered (tab woke up), checking peer:', socketId);

      const peerName = userData?.name || 'Participant';
      const oldPeerData = peersRef.current.find(p => p.socketId === socketId);
      let oldOdId = userData?.id || null;
      let oldAvatar = userData?.avatar || null;

      if (oldPeerData) {
        console.log('  ⚠️ Tearing down existing peer to renegotiate with recovered participant');
        if (!oldOdId) oldOdId = oldPeerData.odId;
        if (!oldAvatar) oldAvatar = oldPeerData.avatar;
        try { oldPeerData.peer.destroy(); } catch (e) { }
      ...
      if (shouldInitiatePeer(socketId)) {
        setTimeout(() => {
          if (hasJoinedRoom.current && socket.connected) {
            const peer = createPeer(socketId, true, peerName);
            if (peer) {
              addPeer(socketId, peer, peerName, oldOdId, oldAvatar, true);
            }
          }
        }, 1000); // Small delay to let old peer destroy events settle
      }
    };
    ...
    const handleParticipantLeft = ({ socketId }) => {
      ...
      dcRemovePeer(socketId); // close DataChannel for this peer
      setParticipants(prev => prev.filter(p => p.socketId !== socketId));
    };

    const handleSocketMigrated = ({ oldSocketId, newSocketId, id, odId }) => {
      console.log(`🔄 Participant morphed socket ID from ${oldSocketId} to ${newSocketId} (User ${odId})`);

      // Update our local maps so future WebRTC signaling uses the NEW socket ID
      const name = socketToName.get(oldSocketId);
      if (name) {
        socketToName.delete(oldSocketId);
        socketToName.set(newSocketId, name);
      }
      ...
      if (peerFound) {
        ...
        console.log(`✅ Successfully seamlessly migrated WebRTC peer to new signaling socket!`);
      }
    };
    ...
    const handleSignal = ({ signal, from, userName, userId, userAvatar }) => {
      ...
      if (existingPeer) {
        ...
        if (existingPeer.peer.destroyed) {
          ...
          peersRef.current = peersRef.current.filter(p => p.socketId !== getLatestPeerId(from || p.socketId, p.peer || peer));
      ...
      peer.on("signal", sig => {
        const currentTargetId = getLatestPeerId(from, peer);
        console.log('📡 Sending signal to:', currentTargetId, 'type:', sig.type, 'myName:', myName);
      ...
      peer.on('stream', remoteStream => {
        const currentTargetId = getLatestPeerId(from, peer);
        console.log('📺 RECEIVED STREAM from:', currentTargetId, 'streamId:', remoteStream.id, 'tracks:', remoteStream.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled })));
        const peerRef = peersRef.current.find(p => p.socketId === getLatestPeerId(from, peer));
        if (peerRef) {
          peerRef.stream = remoteStream;
          console.log('  ✅ Updated peerRef.stream');
        }
        setPeers(prev => {
          const updated = prev.map(p =>
            p.socketId === getLatestPeerId(from, peer) ? { ...p, stream: remoteStream } : p
          );
          console.log('  ✅ Updated peers state');
          return updated;
        });
      ...
      peer._pc.addEventListener('iceconnectionstatechange', () => {
        ...
        if (state === 'failed') {
          console.warn(`⚠️ ICE connection failed for ${from}, scheduling recovery`);

          let oldOdId = userId || null;
          let oldAvatar = userAvatar || null;
          const oldPeerData = peersRef.current.find(p => p.socketId === from);
          if (oldPeerData) {
            if (!oldOdId) oldOdId = oldPeerData.odId;
            if (!oldAvatar) oldAvatar = oldPeerData.avatar;
          }
          ...
          if (shouldInitiatePeer(getLatestPeerId(from, peer))) {
            setTimeout(() => {
              if (hasJoinedRoom.current && socket.connected) {
                console.log(`🔄 Attempting to recreate peer for ${from} after ICE failure`);
                const resolvedId = getLatestPeerId(from, peer);
                const newPeer = createPeer(resolvedId, true, peerName);
                if (newPeer) addPeer(resolvedId, newPeer, peerName, oldOdId, oldAvatar, true);
              }
            }, 2000);
          }
        }
      ...
      try {
        addPeer(from, peer, peerName, userId || null, userAvatar || null, false);
    ...
    // Socket reconnect listener - When we reconnect after tab backgrounding
    ...
    const handleRoomError = ({ message }) => {
      ...
      setActiveAlert({
        title: 'Room Error',
        message: message || 'An unexpected error occurred in the room.',
        type: 'error',
    ...
    const registerListeners = () => {
      ...
      socket.on('participant:socket_migrated', handleSocketMigrated);
      // socket.on('participant:recovered', handleParticipantRecovered); // Removed aggressive teardown
    ...
    // Send room:leave only on actual page unload (tab close / navigation away)
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // Do NOT blindly rejoin or force peers to drop here! 
        // Only attempt to start playing local video if it was paused
        if (userVideo.current && userVideo.current.paused) {
          userVideo.current.play().catch(e => console.warn('Local video play failed:', e));
        }

        // Let ICE connection state handlers manage dead peers, instead of aggressively emitting join
      }
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
    ...
    return () => {
      ...
      socket.off('participant:socket_migrated', handleSocketMigrated);
      socket.off('participant:recovered', handleParticipantRecovered);
      ...
      peersRef.current = [];
      setPeers([]);
      // Don't send room:leave here — the server's grace period handles reconnects.
      // room:leave is sent via beforeunload or via the explicit Leave Room button.
      window.removeEventListener('beforeunload', handleBeforeUnload);
      document.removeEventListener('visibilitychange', handleVisibilityChange);
    };
  }, [roomId, user, socket, profile]); // Added profile to dependencies

  // --- 2.5 Re-join room after socket reconnects (tab switch / network drop) ---
  // reconnectCount increments every time the socket successfully reconnects.
  // We use getSocket() instead of the `socket` closure value so we always emit
  // on the *new* live socket, not the stale one captured at mount time.
  useEffect(() => {
    if (reconnectCount === 0 || !roomId || !user) return;

    const liveSocket = getSocket();
    if (!liveSocket || !liveSocket.connected) return;

    console.log(`🔄 Socket reconnected (count: ${reconnectCount}), re-joining room ${roomId}`);
    ...
    // Re-add our local stream to any existing peers that survived the reconnect
    if (streamRef.current) {
      peersRef.current.forEach(({ peer, socketId }) => {
        if (peer && !peer.destroyed) {
          try {
            streamRef.current.getTracks().forEach(track => {
              // Only add if not already in the sender list
              const senders = peer._pc?.getSenders?.() || [];
              const alreadySending = senders.some(s => s.track?.id === track.id);
              if (!alreadySending) peer.addTrack(track, streamRef.current);
            });
          } catch (e) {
            console.warn('Could not re-add track to peer after reconnect:', socketId, e.message);
          }
        }
      });
    }
  ...
  // --- 2.6 Sync Movie Details (Host Only) ---
  useEffect(() => {
    ...
    socket.on('sync:play', ({ timestamp }) => {
      ...
      const applyPlay = () => {
        ...
        videoRef.current.play()
          ...
          .catch(err => {
            console.warn('Guest play failed:', err);
            isSyncingRef.current = false;
          });
      };
      if (videoRef.current) {
  ...
  // ── DataChannel incoming message handlers ─────────────────────────────
  // Mirror every real-time event that previously came only through Socket.io.
  // These fire even when the tab is backgrounded (browser media priority).
  useEffect(() => {
    // ── Playback sync ──
    const onDCPlay = ({ timestamp }) => {
      if (isHostRef.current) return;
      if (!videoRef.current) return;
      isSyncingRef.current = true;
      videoRef.current.currentTime = timestamp;
      setCurrentTime(timestamp);
      videoRef.current.play()
        .then(() => { setIsPlaying(true); isSyncingRef.current = false; })
        .catch(() => { isSyncingRef.current = false; });
    };

    const onDCPause = ({ timestamp }) => {
      if (isHostRef.current) return;
      if (!videoRef.current) return;
      isSyncingRef.current = true;
      videoRef.current.currentTime = timestamp;
    ...
    const onDCSeek = ({ timestamp }) => {
      if (isHostRef.current || !videoRef.current) return;
      videoRef.current.currentTime = timestamp;
      setCurrentTime(timestamp);
    };

    const onDCSpeed = ({ speed }) => {
      if (isHostRef.current || !videoRef.current) return;
      videoRef.current.playbackRate = speed;
      setPlaybackSpeed(speed);
    };

    const onDCQuality = ({ quality }) => {
      if (!isHostRef.current) setQuality(quality);
    };

    // ── Media status ──
    const onDCMedia = ({ userId, type, enabled }) => {
      if (type === 'audio') {
        setMutedParticipants(prev => { const s = new Set(prev); enabled ? s.delete(userId) : s.add(userId); return s; });
      } else if (type === 'video') {
        setVideoOffParticipants(prev => { const s = new Set(prev); enabled ? s.delete(userId) : s.add(userId); return s; });
      }
    };

    // ── Buffering ──
    const onDCBuffering = ({ odId, isBuffering }) => {
      // Re-use existing socket handler logic by dispatching synthetic event to video
      // (server-side buffering pause logic runs server-side; DC path just updates UI)
      console.log(`[DC] buffering from ${odId}: ${isBuffering}`);
    };

    dcOn('sync:play',       onDCPlay);
    dcOn('sync:pause',      onDCPause);
    dcOn('sync:seek',       onDCSeek);
    dcOn('sync:speed',      onDCSpeed);
    dcOn('sync:quality',    onDCQuality);
    // NOTE: chat:message intentionally NOT registered here — chat goes via Socket.io only
    dcOn('media:status',    onDCMedia);
    dcOn('sync:buffering',  onDCBuffering);

    return () => {
      dcOff('sync:play',      onDCPlay);
      dcOff('sync:pause',     onDCPause);
      dcOff('sync:seek',      onDCSeek);
      dcOff('sync:speed',     onDCSpeed);
      dcOff('sync:quality',   onDCQuality);
      dcOff('media:status',   onDCMedia);
      dcOff('sync:buffering', onDCBuffering);
    };
  }, [dcOn, dcOff, addToast, user?.id]);
  ...
  useEffect(() => {
    ...
    if (isHost) {
      ...
      const onWaiting = () => {
        console.log('⏳ Host video buffering...');
        setIsVideoLoading(true);
        // Debounce buffering signal to avoid rapid state changes
        clearTimeout(bufferingTimeout);
        bufferingTimeout = setTimeout(() => {
          roomEmit('sync:buffering', { odId: user?.id, isBuffering: true });
        }, 200);
      };
      const onPlaying = () => {
        console.log('▶️ Host video playing...');
        setIsVideoLoading(false);
        clearTimeout(bufferingTimeout);
        roomEmit('sync:buffering', { odId: user?.id, isBuffering: false });
      };
      const onCanPlay = () => {
        console.log('✅ Host video can play...');
        setIsVideoLoading(false);
        // Only emit if we were previously buffering
        clearTimeout(bufferingTimeout);
      };
      ...
      const handleBufferPause = ({ message }) => {
        ...
        // Force pause the video regardless of current state
        if (videoRef.current) {
          const video = videoRef.current;
          console.log('  Current video state - paused:', video.paused, 'readyState:', video.readyState);

          if (!video.paused) {
            isSyncingRef.current = true;
            video.pause();
            setIsPlaying(false);
            console.log('  ✅ Guest video paused due to host buffering');
            setTimeout(() => { isSyncingRef.current = false; }, 100);
          } else {
            console.log('  ℹ️ Guest video already paused');
          }
        }
      };
      ...
      const handleBufferResume = () => {
        ...
        if (videoRef.current) {
          ...
          // Only resume if video is paused and ready
          if (video.paused && video.readyState >= 2) {
            isSyncingRef.current = true;
            video.play()
              .then(() => {
                setIsPlaying(true);
                console.log('  ✅ Guest video resumed after host buffering');
                setTimeout(() => { isSyncingRef.current = false; }, 100);
              })
              .catch(err => {
                console.warn('  ❌ Guest resume failed:', err);
                setTimeout(() => { isSyncingRef.current = false; }, 100);
              });
          } else if (!video.paused) {
            console.log('  ℹ️ Guest video already playing');
          } else {
            console.log('  ⚠️ Guest video not ready to play, readyState:', video.readyState);
          }
        }
      };

      // Also track local guest buffering
      const onGuestWaiting = () => {
        console.log('⏳ Guest video buffering...');
        setIsVideoLoading(true);
      };
      const onGuestPlaying = () => {
        console.log('▶️ Guest video playing...');
        setIsVideoLoading(false);
      };
      const onGuestCanPlay = () => {
        console.log('✅ Guest video can play...');
        setIsVideoLoading(false);
      };
  ...
  const handleSendMessage = (e) => {
    ...
    // Chat ALWAYS goes through Socket.io — the server broadcasts to all
    // room members (including those without a DataChannel) and stores
    // history in room.chat[]. P2P via DataChannel would silently drop
    // messages for any participant whose WebRTC isn't fully established.
    socket?.emit('chat:message', { roomId, message: msg });

    // Note: do NOT add msg to local state here.
    // The server echoes back via io.to(roomId).emit('chat:message') which
    // fires socket.on('chat:message') on every client including the sender,
    // so it gets added exactly once by that handler.
    setMessageInput('');
  ...
  const toggleMedia = async (type) => {
    if (type === 'video') {
      if (isVideoEnabled) {
        ...
        roomEmit('media:status', { userId: user.id, type: 'video', enabled: false });
      } else {
        ...
        try {
          ...
          roomEmit('media:status', { userId: user.id, type: 'video', enabled: true });
        } catch (err) {
      ...
      if (stream) {
        ...
        if (audioTrack) {
          ...
          roomEmit('media:status', { userId: user.id, type: 'audio', enabled: newEnabled });
        }
      }
    }
  };
  ...
  const togglePlay = async () => {
    ...
    if (!video.paused) {
      ...
      video.pause();
      setIsPlaying(false);
      roomEmit('sync:pause', { timestamp: video.currentTime });
      setTimeout(() => { isSyncingRef.current = false; }, 100);
    } else {
      ...
      try {
        await video.play();
        setIsPlaying(true);
        roomEmit('sync:play', { timestamp: video.currentTime });
      } catch (err) {
        console.error('Host play failed:', err);
      } finally {
        setTimeout(() => { isSyncingRef.current = false; }, 100);
      }
    }
  ...
  const skip = (seconds) => {
    ...
    roomEmit('sync:seek', { timestamp: newTime });
  };
  ...
  const handleQualityChange = (q) => {
    ...
    roomEmit('sync:quality', { quality: q });
  };
  ...
  const inviteFriend = async (friend) => {
    ...
    try {
      await db.createNotification(friend.id, 'party_invite', {
        ...
        room_id: roomId,
        movie_id: movieId,
        content_type: movieData?.type || searchParams.get('type') || 'movie',
        season: movieData?.currentSeason || searchParams.get('season') || null,
        episode: movieData?.currentEpisode || searchParams.get('episode') || null,
      });
    ...
    socket.emit('party:invite', {
      ...
      contentType: movieData?.type || searchParams.get('type') || 'movie',
      season: movieData?.currentSeason || searchParams.get('season') || null,
      episode: movieData?.currentEpisode || searchParams.get('episode') || null,
      ...
      >
        ...
        >
          <video
            ...
            onPlay={() => {
              if (!isSyncingRef.current && !isHostBuffering) {
                setIsPlaying(true);
              }
            }}
            onPause={() => {
