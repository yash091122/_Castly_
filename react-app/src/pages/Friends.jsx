...
function Friends() {
    const { user } = useAuth();
    const { friends, pendingRequests, sentRequests, sendFriendRequest: contextSendFriendRequest, acceptRequest, rejectRequest, removeFriend, getOnlineFriends, refreshFriends } = useFriends();
    ...
    const sendFriendRequest = async (friendId) => {
        // Find the user from search results or suggestions to pass full user object to context
        const friendUser = searchResults.find(u => u.id === friendId) || suggestions.find(u => u.id === friendId);
        if (!friendUser) return;

        // Use context's sendFriendRequest which handles socket notification, sentRequests tracking, and DB notification
        const result = await contextSendFriendRequest(friendUser);
        if (result) {
            // Don't remove from search results — let the UI show "Request Sent" state via sentRequests.includes()
                                ...
                                {loading && searchQuery ? (
                                    ...
                                    searchResults.length === 0 ? (
                                        ...
                                        searchResults.map(result => {
                                            const isFriend = friends.some(f => String(f.id) === String(result.id));
                                            const hasIncomingRequest = pendingRequests.some(r => String(r.user_id) === String(result.id));
                                            const hasSentRequest = sentRequests.some(id => String(id) === String(result.id));
